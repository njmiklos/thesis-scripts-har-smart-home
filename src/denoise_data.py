...
from handle_csv import get_all_csv_files_in_directory, save_pandas_dataframe_to_csv
from get_env import get_base_path
from infer_sensor_metadata import infer_precision
from explore_data_pandas import calculate_thresholds


def apply_low_pass_filter(df: pd.DataFrame, col: str, cutoff_freq: float, fs: float) -> pd.DataFrame:
    """
    Applies a low-pass Butterworth filter to a specified column in a DataFrame.

    Parameters:
    - df (pd.DataFrame): DataFrame containing the data.
    - col (str): Column name to apply the filter.
    - cutoff_freq (float): Cutoff frequency of the low-pass filter.
    - fs (float): Sampling frequency of the data.

    Returns:
    - pd.DataFrame: DataFrame with the filtered column.
    """
    nyquist = 0.5 * fs
